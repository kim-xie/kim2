(function($){
	$.fn.tmTip = function(options){
		...
		function tipInit($this,opts){
			...
			$("body").append('<div class="tm-tips "><div class="tm-window-tip tooltip-nightly animated bounceIn"><div id="tm-tip-content"></div></div><div class="tooltip-nightly-arrow animated flip"></div><div>');
...
$.loading = function(options){
	...
	var mark = Math.floor(Math.random() * 10);
...
$.tzPrompt = function(options){
	...
	if(opts.textarea){
		...
		$dialog.find(".tzdialog_message").html("<div style='width:"+mw+"px;margin:0 auto;text-align:left'>" +
				"<p><textarea class='dtext' style='overflow:auto;line-height:22px;font-size:12px;width:"+mw+"px;height:"+mh+"px;border:1px solid #ccc;' placeholder='请输入内容...'>"+opts.value+"</textarea></p></div>");
		
	}else{
		$dialog.find(".tzdialog_message").html("<div style='width:"+mw+"px;margin:0 auto;text-align:left'><p>&nbsp;&nbsp;"+opts.content+":</p>" +
				"<p><input type='text'  class='dtext' style='width:"+mw+"px;height:36px;line-height:36px;text-indent:0.5em;border:1px solid #ccc;' value='"+opts.value+"' placeholder='请输入内容...'></p></div>");
	}
